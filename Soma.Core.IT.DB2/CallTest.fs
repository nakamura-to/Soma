...
module CallTest = 
  ...
  let ``call with ADO.NET fashion : ProcMultiParams``() =
    ...
    let factory = Db2.config.DbProviderFactory
    use connection = factory.CreateConnection()
    connection.ConnectionString <- Db2.config.ConnectionString
    connection.Open()
  ...
  let ``call : ProcNoneParam``() =
    use ts = new TransactionScope()
    let result = Db2.call<ProcNoneParam> { Unit = () }
    ()

  [<Test>]
  let ``call : ProcSingleParam``() =
    use ts = new TransactionScope()
    let result = Db2.call<ProcSingleParam> { Param1 = 10 }
    assert_equal 10 result.Param1

  [<Test>]
  let ``call : PorcMultiParams``() =
    use ts = new TransactionScope()
    let result = Db2.call<ProcMultiParams> { Param1 = 1; Hoge = 2; Param3 = 3 }
    assert_equal { ProcMultiParams.Param1 = 1; Hoge = 3; Param3 = 1} result

  [<Test>]
  let ``call : ProcEntityResult``() =
    use ts = new TransactionScope()
    let result = Db2.call<ProcEntityResult> { EmployeeId = 1; EmpList = [] }
  ...
  let ``call : ProcEntityResizeArrayResult``() =
    use ts = new TransactionScope()
    let result = Db2.call<ProcEntityResizeArrayResult> { EmployeeId = 1; EmpList = ResizeArray() }
    assert_equal 1 result.EmployeeId
  ...
  let ``call : ProcTupleResult``() =
    use ts = new TransactionScope()
    let result = Db2.call<ProcTupleResult> { EmployeeId = 1; EmpList = [] }
    assert_equal 1 result.EmployeeId
    assert_equal 3 result.EmpList.Length
  ...
  let ``call : ProcEntityResultAndOut``() =
    use ts = new TransactionScope()
    let result = Db2.call<ProcEntityResultAndOut> { EmployeeId = 1; EmpList = [] ; EmployeeCount = 0; }
    printfn "%A" result
    assert_equal 1 result.EmployeeId
    assert_equal 3 result.EmpList.Length
  ...
  let ``call : ProcTupleResultAndOut``() =
    use ts = new TransactionScope()
    let result = Db2.call<ProcTupleResultAndOut> { EmployeeId = 1; EmpList = [] ; EmployeeCount = 0; }
    printfn "%A" result
    assert_equal 1 result.EmployeeId
    assert_equal 3 result.EmpList.Length
    assert_equal 4 result.EmployeeCount
  ...
  let ``call : ProcResultAndUpdate``() =
    use ts = new TransactionScope()
    let result = Db2.call<ProcResultAndUpdate> { EmployeeId = 1; EmpList = [] ; }
    assert_equal 1 result.EmployeeId
    assert_equal 3 result.EmpList.Length
    let dept = Db2.find<Department> [1]
    assert_equal "HOGE" dept.DepartmentName

  [<Test>]
  let ``call : ProcUpdateAndResult``() =
    use ts = new TransactionScope()
    let result = Db2.call<ProcUpdateAndResult> { EmployeeId = 1; EmpList = [] ; }
    assert_equal 1 result.EmployeeId
    assert_equal 3 result.EmpList.Length
    let dept = Db2.find<Department> [1]
    assert_equal "HOGE" dept.DepartmentName
  ...
  let ``call : ProcEntityResults``() =
    use ts = new TransactionScope()
    let result = Db2.call<ProcEntityResults> { EmployeeId = 1; DepartmentId = 1; EmpList = [] ; DeptList = [] }
    assert_equal 1 result.EmployeeId
  ...
  let ``call : ProcTupleResults``() =
    use ts = new TransactionScope()
    let result = Db2.call<ProcTupleResults> { EmployeeId = 1; DepartmentId = 1; EmpList = [] ; DeptList = [] }
    assert_equal 1 result.EmployeeId
    assert_equal 1 result.DepartmentId
    assert_equal 3 result.EmpList.Length
    assert_equal 1 result.DeptList.Length
