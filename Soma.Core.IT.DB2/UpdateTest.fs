...
module UpdateTest = 
  ...
  let ``update : no id``() =
    ...
    try
      Db2.update { NoId.Name = "aaa"; VersionNo = 0 } |> ignore
      fail ()
    with 
  ...
  let ``update : composite id``() =
    ...
    let employee = Db2.update { employee with EmployeeName = "hoge" }
    printfn "%A" employee
  ...
  let ``update : no version``() =
    use ts = new TransactionScope()
    let noVersion = Db2.update { NoVersion.Id = 1; Name = "aaa" }
    printfn "%A" noVersion
  ...
  let ``update : incremented version``() =
    ...
    let department = Db2.update department
  ...
  let ``update : Enum``() =
    use ts = new TransactionScope()
    let person = Db2.find<Person> [2]
    let person = Db2.update { person with JobKind = JobKind.Salesman }
    printfn "%A" person
  ...
  let ``updateIgnoreVersion``() =
    use ts = new TransactionScope()
    let department = { DepartmentId = 1; DepartmentName = "hoge"; VersionNo = -1 }
    let department = Db2.updateWithOpt department (UpdateOpt(IgnoreVersion = true))
    printfn "%A" department
  ...
  let ``updateIgnoreVersion : no affected row``() =
    ...
    try
      Db2.updateWithOpt department (UpdateOpt(IgnoreVersion = true)) |> ignore
      fail ()
    with 
