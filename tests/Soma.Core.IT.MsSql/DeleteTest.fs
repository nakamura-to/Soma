...
module DeleteTest = 
  ...
  let ``delete : no id``() =
    ...
    try
      MsSql.delete { NoId.Name = "aaa"; VersionNo = 0 } |> ignore
      fail ()
    with 
  ...
  let ``delete : no version``() =
    use ts = new TransactionScope()
    MsSql.delete { NoVersion.Id = 1; Name = "aaa" }
  ...
  let ``delete : computed version``() =
    use ts = new TransactionScope()
    let department = MsSql.insert { AddressId = 0; Street = "hoge"; VersionNo = Array.empty }
    MsSql.delete department
